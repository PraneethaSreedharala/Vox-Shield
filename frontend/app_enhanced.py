...
def create_frequency_plot(title, data_color):
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy',
        fillcolor=f'rgba({data_color[4:-1]}, 0.3)'
    ))
...
with col2:
    ...
    st.plotly_chart(protected_plot, use_container_width=True=True, key="protected")
...
def update_visualization():
    ...
    if freqs is not None and raw_magnitude is not None and processed_magnitude is not None:
        ...
        # Update stats
        stats_placeholder.metric("Processing Intensity", f"{spectral_diff:.4f}")
        stats_placeholder.metric("Energy Ratio", f"{energy_ratio:.3f}")
        
        # Get update statistics
        update_stats = audio_data_share.get_update_stats()
        stats_placeholder.metric("Updates/sec", f"{update_stats['updates_per_second']:.1f}")
